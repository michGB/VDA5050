...
[7.3 Rotation on the spot](#73-rotation-on-the-spot)<br>
...
*length* | m | float64 | Length of the path from the start node to the end node. Edges with a length of 0 are explicitly valid edges, which can be used, for example, for rotations on the spot.<br><br>Optional:<br>This value is used by line-guided AGVs to decrease their speed before reaching a stop position.
